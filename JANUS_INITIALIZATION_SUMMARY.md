...
1. **Branch Merging**: Implement logic to merge branches back into `main`, either via a full rebase or a squashed merge.
2. **Persistence Adapters**: Develop adapters for file-based (JSON/Parquet) or database-backed state persistence for long-running workflows.

### Phase 3: Research & Ecosystem

1. **Tester Branches**: Explore "Sandboxed Execution" where operations are performed on a temporary branch and discarded if specific conditions aren't met.
2. **Ecosystem Plugins**: Prototype adapters for `numpy.ndarray` and `pandas.DataFrame` to showcase the power of the plugin architecture.
